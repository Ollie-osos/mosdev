...
module.exports = options => {
  ...
  var allModules = fs
    .readdirSync(`${libPath}/V4`)
    .filter(item => /-native\.js$/.test(item));

  // Get a list of modules to include:
  var modules = [];
  if (options.only) {
    // If --only, use that for modules
    ...
    // If neither --ignore or --only, use allModules
  ...
  modules.forEach(function(name, i) {
    promises[i] = new Promise(function(resolve, reject) {
      fs.readFile(
        `${libPath}/V4/${name.toLowerCase()}-native.js`,
        'utf8',
  ...
  var result = Promise.all(promises)
    .then(function(modules) {
      var header =
        '// Native Javascript for Bootstrap 4 ' +
        version +
        ' | © dnp_theme | ' +
        license +
        '\n';
  ...
  function wrap(main) {
    var utils = fs
      .readFileSync(`${libPath}/V4/utils.js`, 'utf8')
      .split('\n')
      .join('\n  ');
    var init =
      options.autoInitDataAPI === false
        ? ''
        : options.noInitDataAPI
          ? ''
          : fs
              .readFileSync(`${libPath}/V4/utils-init.js`, 'utf8')
              .split('\n')
              .join('\n  ');
    main = main.split('\n').join('\n  ');
    // Initialize arrays:
