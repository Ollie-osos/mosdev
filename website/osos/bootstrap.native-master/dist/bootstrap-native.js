...
(function(root, factory) {
  ...
  var globalObject = typeof global !== 'undefined' ? global : this || window,
    ...
    getElementsByClassName = function(element, classNAME) {
      ...
      return nodeListToArray(
        element[selectionMethod](
          isIE8 ? '.' + classNAME.replace(/\s(?=[a-z])/g, '.') : classNAME
        )
      );
    },
    ...
    getTransitionDurationFromElement = function(element) {
      var duration = globalObject[getComputedStyle](element)[
        transitionDuration
      ];
  ...
  var Affix = function(element, options) {
    ...
    var targetData = element[getAttribute](dataTarget),
      ...
      update = 'update',
      affixTop = 'affix-top',
      affixedTop = 'affixed-top',
      affixBottom = 'affix-bottom',
      affixedBottom = 'affixed-bottom';

    this[target] = options[target]
      ? queryElement(options[target])
      : queryElement(targetData) || null; // target is an object
    this[offsetTop] = options[offsetTop]
      ? options[offsetTop]
      : parseInt(offsetTopData) || 0; // offset option is an integer number or function to determine that number
    this[offsetBottom] = options[offsetBottom]
      ? options[offsetBottom]
      : parseInt(offsetBottomData) || 0;
    ...
    var self = this,
      ...
      checkPosition = function() {
        ...
        pinnedTop =
          parseInt(pinOffsetTop) - scrollY < 0 &&
          scrollY > parseInt(pinOffsetTop);
        pinnedBottom =
          parseInt(pinOffsetBottom) - scrollY < 0 &&
          scrollY > parseInt(pinOffsetBottom);
      },
      pinTop = function() {
  ...
  var Button = function(element, option) {
    ...
    if (hasClass(element, 'btn')) {
      // when Button text is used we execute it as an instance method
      if (option !== null) {
        if (option !== reset) {
          setState();
        } else {
      ...
      // if ( hasClass(element,'btn-group') ) // we allow the script to work outside btn-group component

      if (!(stringButton in element)) {
        // prevent adding event handlers twice
        on(element, clickEvent, toggle);
        queryElement('[' + tabindex + ']', element) &&
      ...
      for (var i = 0; i < lbll; i++) {
        !hasClass(labelsToACtivate[i], active) &&
          queryElement('input', labelsToACtivate[i])[getAttribute](checked) &&
  ...
  var Carousel = function(element, options) {
    ...
    var intervalAttribute = element[getAttribute](dataInterval),
      ...
      intervalData =
        intervalAttribute === 'false' ? 0 : parseInt(intervalAttribute),
    ...
    this[pause] =
      options[pause] === hoverEvent || pauseData ? hoverEvent : false; // false / hover
    ...
    var pauseHandler = function() {
      ...
      indicatorHandler = function(e) {
        ...
        if (
          eventTarget &&
          !hasClass(eventTarget, active) &&
          eventTarget[getAttribute](dataSlideTo)
        ) {
    ...
    this.slideTo = function(next) {
      ...
      if (activeItem === next) {
        ...
        activeItem < next ||
        (activeItem === 0 && next === total - 1)
      ) {
        slideDirection = self[direction] = left; // next
      } else if (
        activeItem > next ||
        (activeItem === total - 1 && next === 0)
      ) {
      ...
      if (supportTransitions && hasClass(element, 'slide')) {
        ...
        one(slides[next], transitionEndEvent, function(e) {
          var timeout =
            e[target] !== slides[next] ? e.elapsedTime * 1000 + 100 : 20;
          isSliding &&
            setTimeout(function() {
              isSliding = false;
        ...
        setTimeout(function() {
          ...
          bootstrapCustomEvent.call(
            ...
            component,
            slides[next]
          ); // here we go with the slid event
        }, 100);
      }
    };
    this.getActiveIndex = function() {
      return (
        slides[indexOf](getElementsByClassName(element, 'item active')[0]) || 0
      );
    };
  ...
  var Collapse = function(element, options) {
    ...
    this.show = function() {
      ...
      if (
        ...
        (activeCollapse && !activeCollapse[isAnimating])
      ) {
    ...
    accordion =
      queryElement(options.parent) ||
      (accordionData && getClosest(element, accordionData));
    ...
    var self = this,
      ...
      menuItems = (function() {
        ...
        for (var i = 0; i < set[length]; i++) {
          set[i][children][length] &&
            (set[i][children][0].tagName === 'A' && newSet[push](set[i]));
        }
        return newSet;
      })(),
      ...
      dismissHandler = function(e) {
        var eventTarget = e[target],
          hasData =
            eventTarget &&
            (stringDropdown in eventTarget ||
              stringDropdown in eventTarget[parentNode]);
        if (
  ...
  var Modal = function(element, options) {
    ...
    var self = this,
      ...
      setScrollbar = function() {
        var bodyStyle =
            DOC[body].currentStyle || globalObject[getComputedStyle](DOC[body]),
        ...
        if (bodyIsOverflowing) {
          ...
          if (fixedItems[length]) {
            for (var i = 0; i < fixedItems[length]; i++) {
              itemPad = (fixedItems[i].currentStyle ||
                globalObject[getComputedStyle](fixedItems[i]))[paddingRight];
  ...
  var Popover = function(element, options) {
    ...
    this[dismissible] =
      options[dismissible] || dismissibleData === 'true' ? true : false;
      ...
      createPopover = function() {
        ...
        if (contentString !== null && self[template] === null) {
          ...
          if (titleString !== null) {
            ...
            popoverTitle[innerHTML] = self[dismissible]
              ? titleString + closeBtn
              : titleString;
            popover[appendChild](popoverTitle);
          }

          var popoverArrow = DOC[createElement](div),
            popoverContent = DOC[createElement](div);
          ...
          popoverContent[innerHTML] =
            self[dismissible] && titleString === null
        ...
        popover[setAttribute](
          classString,
          component + ' ' + placementSetting + ' ' + self[animation]
        );
      },
        ...
        if (!isActive && inside) {
          ...
          if (parent.tagName === 'LI' && hasClass(parent, active)) {
            ...
            if (
              ...
              !getElementsByClassName(parent[parentNode], active).length
            ) {
  ...
  var Tab = function(element, options) {
    ...
    var getActiveTab = function() {
      ...
      clickHandler = function(e) {
        ...
        e[preventDefault]();
        next =
          e[target][getAttribute](dataToggle) === component ||
          (href && href.charAt(0) === '#')
            ? e[target]
            : e[target][parentNode]; // allow for child elements like icons to use the handler
        !tabs[isAnimating] &&
          !hasClass(next[parentNode], active) &&
          self.show();
      };
  ...
  var Tooltip = function(element, options) {
    ...
    var self = this,
      ...
      titleString =
        ...
        element[getAttribute](dataTitle) ||
        element[getAttribute](dataOriginalTitle);
    ...
    var removeToolTip = function() {
      ...
      createToolTip = function() {
        titleString =
          element[getAttribute](title) ||
          element[getAttribute](dataTitle) ||
          element[getAttribute](dataOriginalTitle); // read the title again
        ...
        var tooltipArrow = DOC[createElement](div),
          tooltipInner = DOC[createElement](div);
        tooltipArrow[setAttribute](classString, component + '-arrow');
